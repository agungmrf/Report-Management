...
class Login extends CI_Controller
{
	...
	function logout()
	{
		...
		echo "<script>alert('Berhasil Logout dari Sistem'); window.location = '" . base_url() . "'</script>";
	}

	function dashboard()
	{
		if ($this->session->userdata('username') == null) {
			echo "<script>alert('Anda Belum Login!'); window.location = '" . base_url('index.php') . "'</script>";
		} else {
